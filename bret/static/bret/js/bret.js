...
(function(angular){
	...
	{
		// flip only selected boxes
		for (var i = 0; i < this.collection.length; i++) {
			this.collection[i].$$resolved = true;
		}

		// make sure bomb is also flipped
		this.bomb.$$resolved = true;

		// set global resolved flag (needed for bomb class)
		this.resolved = true;

		this._persist();
	};

	BombTaskController.prototype.stopAndResolve = function()
	{
		// Stop the round
		this.stop();
		...
		// Add a flag to mark if bomb was selected
		this.bomb.$$selected = !!this.bomb.$$active;
